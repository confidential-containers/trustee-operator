...
# Release preparation wrapper script
# Handles version, bundle, git, and PR workflow; images are built by GitHub Actions
...
# Build arguments array to properly handle values with spaces/special chars
RELEASE_ARGS=("${NEW_VERSION}" --fork "${FORK_REPO}")
[[ -n "${BRANCH}" ]] && RELEASE_ARGS+=(--branch "${BRANCH}")
[[ "${SKIP_TESTS}" == "true" ]] && RELEASE_ARGS+=(--skip-tests)
[[ "${SKIP_PUSH}" == "true" ]] && RELEASE_ARGS+=(--skip-push)
[[ "${SKIP_PR}" == "true" ]] && RELEASE_ARGS+=(--skip-pr)
[[ "${DRY_RUN}" == "true" ]] && RELEASE_ARGS+=(--dry-run)

if [[ "${DRY_RUN}" == "true" ]]; then
    log_info "[DRY RUN] Would run: ${SCRIPT_DIR}/release.sh ${RELEASE_ARGS[*]}"
else
    bash "${SCRIPT_DIR}/release.sh" "${RELEASE_ARGS[@]}" || {
