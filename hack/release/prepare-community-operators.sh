...
Optional Options:
  ...
  --work-dir DIR     Working directory (default: \${TMPDIR:-/tmp}/community-operators-\${VERSION})
                     NOTE: Must be under system temp directory for safety (enforced before rm -rf)
                           Uses \$TMPDIR if set (common on macOS), otherwise /tmp
...
What this script does:
  ...
  3. Creates branch and copies bundle files
  4. Creates commit
  5. Pushes to fork and creates PR (unless --skip-push or --skip-pr)
EOF
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --upstream)
            if [[ $# -lt 2 ]] || [[ -z "${2:-}" ]]; then
                log_error "Missing value for --upstream"
                usage
                exit 1
            fi
            UPSTREAM_REPO="$2"
        ...
        --catalog)
            if [[ $# -lt 2 ]] || [[ -z "${2:-}" ]]; then
                log_error "Missing value for --catalog"
                usage
                exit 1
            fi
            CATALOG="$2"
        ...
        --work-dir)
            if [[ $# -lt 2 ]] || [[ -z "${2:-}" ]]; then
                log_error "Missing value for --work-dir"
                usage
                exit 1
            fi
            WORK_DIR="$2"
...
# This is needed because we allow --work-dir to start with /tmp even when TMPDIR is set
TMP_CANONICAL=""
if [[ -d "/tmp" ]]; then
...
if [[ "${SKIP_CLONE}" == "false" ]]; then
    ...
    if [[ "${DRY_RUN}" == "false" ]]; then
        ...
        if ! git rev-parse --git-dir &>/dev/null; then
            log_error "Work directory is not a git repository: ${WORK_DIR}"
            log_error "Either remove --skip-clone to clone fresh, or provide a valid git repo"
            exit 1
        fi

        # Ensure fork remote exists and points to correct URL
        if git remote get-url fork &>/dev/null; then
            log_info "Fork remote already exists, updating URL..."
            git remote set-url fork "${FORK_REPO}"
        else
        ...
        log_info "Creating/checking out branch: ${BRANCH}"
        if git show-ref --verify --quiet refs/heads/"${BRANCH}"; then
            log_info "Branch ${BRANCH} already exists, checking it out"
            git checkout "${BRANCH}" || {
        ...
        log_info "[DRY RUN] Would verify ${WORK_DIR} is a git repo"
        log_info "[DRY RUN] Would add/update fork remote: ${FORK_REPO}"
        log_info "[DRY RUN] Would create/checkout branch: ${BRANCH}"
    fi
    echo ""
fi
