...
# Remove duplicate controller image entries from kustomization.yaml
...
remove_duplicate_kustomization_images() {
    ...
    # Check for duplicate "name: controller" entries
    local controller_count
    controller_count=$(grep -c "^[[:space:]]*- name: controller" "${kustomization_file}" 2>/dev/null || echo "0")
    ...
    if [[ "${controller_count}" -gt 1 ]]; then
        ...
        # Use awk to skip the second occurrence of the entire image block
        awk '
            /^[[:space:]]*-[[:space:]]+name:[[:space:]]+controller[[:space:]]*$/ {
                count++
                if (count > 1) {
                    skip = 1
                    next
