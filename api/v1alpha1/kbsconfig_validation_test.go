	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.tlsConfig.MinVersion != "" && tt.tlsConfig.MaxVersion != "" {
				if tt.tlsConfig.MinVersion > tt.tlsConfig.MaxVersion {
					if tt.wantValid {
						t.Errorf("minVersion > maxVersion should fail validation")
					}
				} else {
					if !tt.wantValid && tt.wantErrMsg == "minVersion must not be greater than maxVersion" {
						t.Errorf("minVersion <= maxVersion should pass validation")
					}
				}
			}
		})
